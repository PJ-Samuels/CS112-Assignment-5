...
public class Sudoku {    
     ...
     */
    public void readConfig(Scanner input) { //dont change
        for (int r = 0; r < 9; r++) {
            for (int c = 0; c < 9; c++) {
     ...
     * You should not change this method.
     */        
    public void printGrid() { //dont change
        for (int r = 0; r < 9; r++) {
            printRowSeparator();//may need this
            for (int c = 0; c < 9; c++) {
                System.out.print("|");
                if (this.grid[r][c] == 0) {
                    System.out.print("   ");
    ...
    private static void printRowSeparator() {//dont change
        for (int i = 0; i < 9; i++) {
            System.out.print("----");
        }
        System.out.println("-");
     ...
     *    18 ...
     */
    private boolean solveRB(int n) {//n is the 1-80 digit being checked in the is subgridhasValid
        ...
        {
            placeVal(grid[row][col],row,col);
            return this.solveRB(n+1);
        }
        for(int val = 1; val < 10; val++)
        {
            if(!this.subgridHasVal[subRow][subCol][val] && !rowCheck(val,row) && !colCheck(val, col)){
    ...
    public boolean rowCheck(int val, int row)
    {
        for(int i = 0; i < 9; i++)
        {
            if(val == grid[row][i])
    ...
    public boolean colCheck(int val, int col)
    {
        for(int i = 0; i < 9; i++)
        {
            if(val == grid[i][col])
     ...
     */
    public boolean solve() {  //dont change
        boolean foundSol = this.solveRB(0);
        return foundSol;
    }
    
    public static void main(String[] args) {//dont change
